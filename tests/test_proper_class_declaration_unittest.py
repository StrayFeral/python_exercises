...
class TestDummyClassUnittestVersion(unittest.TestCase):
    ...
    def test_plus_operator(self):
        self.assertEqual(
            self.b1 + self.b2, DummyClass(33), "incorrect addition in plus operator."
        )
        self.assertEqual(
            self.b1.x, 11, "incorrect addition in plus operator."
        )  # Value should not be changed
        self.assertEqual(
            self.b2.x, 22, "incorrect addition in plus operator."
        )  # Value should not be changed

    def test_minus_operator(self):
        self.assertEqual(
            self.b2 - self.b1, DummyClass(11), "incorrect addition in plus operator."
        )
        self.assertEqual(
            self.b1.x, 11, "incorrect addition in plus operator."
        )  # Value should not be changed
    ...
    def test_equals_operator(self):
        self.assertEqual(
            self.b1 == self.b3, True, "incorrect operation of the equals operator."
        )
